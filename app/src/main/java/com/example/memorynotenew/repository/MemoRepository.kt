...
class MemoRepository(private val memoDao: MemoDao, private val trashDao: TrashDao) {
    ...
    fun getAllMemos(): Flow<List<Memo>> { return memoDao.getAllMemos() }

    suspend fun insertTrash(trash: Trash) { trashDao.insertTrash(trash) }
    suspend fun deleteTrash(trash: Trash) { trashDao.deleteTrash(trash) }
    fun getAllTrash(): Flow<List<Trash>> { return trashDao.getAllTrash() }
